...
def well_sample(training_image, nr_samples, disp):
    n_x = training_image.shape[0]  # height
    x = np.arange(0,n_x)
    mu_alpha = 50
    sigma_alpha = 20
    mu_beta = 0
    ...
    for i in range(nr_samples):
        ...
        # print('s:', s)
        well_mat = np.ones(np.shape(training_image))*(-1)  # (h w)
        # print('well_mat.shape:', well_mat.shape)
        for i in range(s):
            ...
            # print('inds:', inds)
            # print('sample.astype(int):', sample.astype(int))
            #print(len(inds))
...
if __name__ == '__main__':
    ...
    print('facies_arr.shape:', facies_arr.shape)
    #Plot facies array
    # plt.figure()
    ...
    st = time.time()
    well_sample(facies_arr,nr_samples,disp=False)
    end = time.time()
    print('runtime: {:0.5f}'.format(end - st))
