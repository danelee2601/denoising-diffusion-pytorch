...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert not (type(self) == GaussianDiffusion and net.channels != net.out_dim)
        assert not net.random_or_learned_sinusoidal_cond

        self.net = net
        self.channels = self.net.channels
        self.self_condition = self.net.self_condition
    ...
    def net_predictions(self, x, t, x_self_cond = None, z_q_cond=None, clip_x_start = False):
        net_output = self.net(x, t, x_self_cond, z_q_cond)
        maybe_clip = partial(torch.clamp, min = -1., max = 1.) if clip_x_start else identity

        if self.objective == 'pred_noise':
            pred_noise = net_output
    ...
    def p_mean_variance(self, x, t, x_self_cond = None, z_q_cond=None, clip_denoised = True, dynamic_thresholding = False):
        ...
        preds = self.net_predictions(x, t, x_self_cond, z_q_cond)
        ...
        net_mean, posterior_variance, posterior_log_variance = self.q_posterior(x_start = x_start, x_t = x, t = t)
        return net_mean, posterior_variance, posterior_log_variance, x_start

    @torch.no_grad()
    def p_sample(self, x, t: int, x_self_cond = None, z_q_cond=None):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        net_mean, _, net_log_variance, x_start = self.p_mean_variance(x = x,
                                                                      t = batched_times,
                                                                      x_self_cond = x_self_cond,
                                                                      z_q_cond=z_q_cond)
        noise = torch.randn_like(x) if t > 0 else 0. # no noise if t == 0
        pred_img = net_mean + (0.5 * net_log_variance).exp() * noise
    ...
    def ddim_sample(self, shape, return_all_timesteps = False):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            self_cond = x_start if self.self_condition else None
            pred_noise, x_start, *_ = self.net_predictions(img, time_cond, self_cond, clip_x_start = True)
    ...
    def p_losses(self, x_start, t, z_q_cond, noise = None):
        ...
        if self.self_condition and random() < 0.5:
            with torch.no_grad():
                assert False, "self_condition cannot be used for now. It's not implemented for the conditional model"
                x_self_cond = self.net_predictions(x, t).pred_x_start
                x_self_cond.detach_()

        # predict and take gradient step
        net_out = self.net(x, t, x_self_cond, z_q_cond)
        ...
        loss = self.loss_fn(net_out, target, reduction = 'none')
        loss = reduce(loss, 'b ... -> b (...)', 'mean')

        loss = loss * extract(self.p2_loss_weight, t, loss.shape)
        return loss.mean()

    def forward(self, z_q, z_q_cond, *args, **kwargs):
        """
        Input:
            - z_q: (b d h' w')
            - cond_img: (b c h w)
        ...
        z_q_cond = self.normalize(z_q_cond)

        return self.p_losses(z_q, t, z_q_cond, *args, **kwargs)
...
def save_image(
    ...
    for i in range(n_rows):
        for j in range(n_rows):
            x_cond = X_cond[i]  # (h w)
            axes[i, j].imshow(x_cond)
            axes[i, j].set_xticks([])
            axes[i, j].set_yticks([])

            xhat = Xhat[i]  # (h w)
            axes[i, n_rows+j].imshow(xhat)
...
class Trainer(object):
    def __init__(
        ...
        pretrained_encoder_cond: VQVAEEncoder,
        pretrained_vq_cond: VectorQuantize,
        *,
        train_batch_size = 16,
        ...
        self.pretrained_encoder_cond = pretrained_encoder_cond
        self.pretrained_vq_cond = pretrained_vq_cond
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    z_q, _, _, _ = quantize(z, self.pretrained_vq, return_z_q_before_proj_out=True)
